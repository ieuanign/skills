#!/usr/bin/env node
//
// The execution state machine — phase-execute.js — driven end to end with NO agent dispatched. Its
// one outside dependency is the `agent()` global, which makes it the highest seam available: a
// scripted fake drives every loop, bound and ending for the price of a `node` process.
//
...
// The name says which script failed, which is the first thing a reader of a FAIL line needs and the
// one thing the scenario body no longer shows.
const scenarios = []
