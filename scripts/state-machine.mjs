#!/usr/bin/env node
//
// The pipeline's execution state machine — phase-execute.js — driven end to end with NO agent
// dispatched. Its one outside dependency is the `agent()` global, which makes it the highest seam
// available: a scripted fake drives every loop, bound and ending for the price of a `node` process.
//
...
// scenarios are the behaviour the script is held to, so a failing one is the script's bug until
...
// A label with NO entry is a harness bug, not a dead agent: the phase script catches every throw, so
// an unreported typo here would read as a plausible FAILED ending. Collected and reported as one.

const runPhase = async (args, script) => {
  const calls = []
  const logs = []
  const unscripted = []
  ...
  const pipeline = () => {
    throw new Error('pipeline() is not used by the phase script')
  }
...
// The name carries the script it drives, which is the first thing a reader of a FAIL line needs and
// the one thing the scenario body no longer shows.
const scenarios = []
