---
name: pr-comments
description: Reads a pull request's unresolved comments, classifies each fix or skip for a human's approval, answers every review thread it read, and makes the approved fixes itself in a worktree on that pull request's own branch, pushed once. Use for `/pr-comments <pull request>`, or `/pr-comments auto <pull request>` for an unattended run.
---

# /pr-comments — a pull request's comments, through to a pushed fix

You are the orchestrator and you read from the MAIN worktree. **The fixes are yours to write** — nothing here dispatches an agent and nothing here runs a workflow — and you write them in a worktree attached to the pull request's own head branch.
...
> **Gate suppression.** Step 3 raises its question under `gated` and raises none under `unattended`. It is this run's only gate; what an unattended run writes in place of an answer is Step 3's comment and the messages below.

**Suppression removes the question, not the work.** Every step of Step 3 still runs: every comment is still classified, the table is still rendered, and every thread is still answered. Each question resolves to its unattended answer instead.
...
A gated run has a human watching and sends none. An unattended one has nobody, so it reports itself where the approval request used to be: **one `start` message at intake and exactly one closing message**, paired. **The pairing is one-directional**: every `start` is closed, and a close with no `start` before it is a run that never began. Detail belongs in the comments — Step 3's table or Step 7's conclusion; a message is the one line somebody triages from a phone.
...
**The reason stays**: a token and a link alone would mean opening the pull request to learn anything at all.

Which token closes a run is asked of the whole run: did something deliberately stop, or did something break?

| How the run ended | Token | Its reason clause |
|---|---|---|
| Step 1's read of the comments failed | `failed` | the read's message |
| Step 1 found no unresolved comments, or Step 3 no **fix** row | `halt` | that there is nothing to do, with the row counts where a table was posted |
| Step 4 could not attach the worktree, found the branch diverged, or had no profile key to run on | `halt` | git's refusal, or the key the profile lacks |
| Step 5 stopped short of the commits the table planned | `halt` where it stopped deliberately; `failed` where something broke | what stopped it |
| Step 6 pushed nothing — rejected, or no commit made | `failed` | that nothing was pushed, and why |
...
**The closing message is the run's last act** — Step 8 on every path that reaches it — and otherwise the stopping step itself (1, 3 or 4), after Step 7 and everything else that step does. A run stopped by Step 1's three refusals sends neither: it wrote nothing anywhere, so there is nothing to report and no `start` to pair.

- **A message is `<DEV-LOOP>/notify.sh <<'MSG' … MSG`** — payload on standard input from a quoted heredoc. `<DEV-LOOP>` is the sibling skill folder, `<this-skill-dir>/../dev-loop`: both install paths put skill folders side by side, so no absolute path is written down, and that one file is the only one of dev-loop's this skill ever resolves — `unattended`'s alone, so a `gated` run never resolves the folder at all. It owns the channel contract, so read no channel environment variable, add no configuration check, add no profile key and reimplement no send: an unconfigured channel is already silent inside it and already exits 0. A send that does not happen — the file missing included — is reported and refuses nothing.
...
**A failed write is reported and changes nothing else about the run** — a comment, a reply or a notification alike. The push, the commits and every write that did land are what they would have been, and a failed reply's reason is in the table either way.

## Step 1 — the pull request, then its unresolved comments

Two reads and nothing else: everything on the way to Step 3's gate leaves the pull request exactly as it was.

**The pull request.** `gh pr view <n> --json number,title,url,state,isCrossRepository,headRefName,baseRefName` — one read, which every later step uses. Three things in it stop the run **here**, each making this run's promise — the fixes, pushed to this pull request's own branch — impossible to keep, and discovering one after a human has read a table wastes the reading as well as the read. A run stopped on one has written nothing anywhere.

1. **Any state but `OPEN`** ⇒ stop. The branch of a merged or closed pull request may already be deleted, and pushing to one is not a fix anybody asked for.
2. **`isCrossRepository: true`** ⇒ stop, saying so. The head branch lives on another remote, so the push this skill promises cannot be made from this checkout at all.
3. **`headRefName` is `<DEFAULT>`** ⇒ stop. The one push lands on it, and a pull request opened from the default branch would take that push to the trunk.

**⟨notify⟩ Run start.** All three passed ⇒ under `unattended`, send the `start` message, before the read below and after nothing that could have written anywhere.

**The comments.** `node <this-skill-dir>/read-comments.mjs <n>` — the bundled read, and the only one. It prints one JSON document, `{ pullRequest, comments: [...] }`, whose entries carry the same keys whatever each started as: `origin` (`review-thread`, `review-body` or `issue-comment`), `id`, `author`, `body`, `url`, `createdAt`, `path`, `line`, `originalLine`, `outdated`, `threadId`, `reviewState`.
...
**Under `unattended`, either ending is explained on the pull request** — Step 7's comment, before this step's closing message. A run with nobody watching that stops here otherwise leaves one line in one person's client and nothing at all on the artifact it was pointed at. So the comment names this step, carries the read's message verbatim where it failed, and says the pull request held no unresolved comments where it did not. It is that run's **only** comment: Step 3's table was never reached, so the ceiling above is not raised. A `gated` run writes nothing here — its human read the reason as it happened.

## Step 2 — classify, and show the table
...
`already addressed` is the one whose evidence is not in the comment set: `gh pr view <n> --json commits`, then name a commit that read actually returned — its `oid` short, and its `messageHeadline`. This is a read of the pull request, not a second read of its comments; Step 1's module stays the only source of those. A sha from anywhere else is one nobody can check.

**An `unclassified` row is stated, not buried.** For each, say plainly that the vocabulary did not cover it and what it appeared to ask. No commit is made for one: it is not a fix, and Step 3 counts fix rows.
...
**Ordinals run file by file, ascending by anchor within each.** One file's fixes take adjacent ordinals, ordered by the line each sits at — `originalLine` where `line` is null — and the fixes anchored to no file come last. They are all made in **one worktree, in one pass**: sequential commits in one worktree is the only thing that makes a later fix open a file with the earlier one already in it, and splitting a run's fixes across worktrees or branches races them instead.
...
**The comment cell is an excerpt, never the body** — its opening, cut to one line. The body itself is read whole at Step 5, where the fix is actually made. **Any `|` a cell carries is escaped, and anything wanting a newline goes to its expansion** — the excerpts cut for evidence included, since one unescaped pipe silently eats the rest of a row.

| Intent | Its clause |
|---|---|
| **fix** | what the fix will do — written once here, and the same string Step 5 implements against and Step 7's reply carries |
...
No cell carries this: the **Why** cell states what a fix will do and the **Reason** cell belongs to skips, and the ordinals alone show only *that* two rows merged, never why. **A shared ordinal with no expansion is an unfinished table**, on the same ground a silent merge is refused above. An expansion that only locates the rows has not given a reason.

**This is the only table definition there is.** Step 3 below — shown at the gate or posted in its place — and Step 7's comment each render *this* table, expansions included: the same rows, columns and clauses, never a second summary of them.

## Step 3 — the gate, and what stands in for it

**Nothing on the way to this line touched the pull request** — every command so far was a read, and unattended's `start` message goes to a channel rather than to any artifact. The one write above it belongs to a run that never arrives here: an unattended Step 1 ending posts its one explanation and stops.

Render Step 2's table and its expansions, and count its **fix** rows. That count decides how much work follows the gate; it never decides whether the threads are answered. **Every path past this line answers them**, whatever the count: the replies are what this run owes the people who wrote the comments, and a run ending for want of code to write owes them just the same.
...
Nobody is going to correct a row, so Step 2's table as classified is the one that counts and the one everything downstream renders. It goes on the pull request, because a table nobody was watching is a decision made in a terminal that closes. Posted per **How this run writes**, it holds Step 2's table and every expansion, rendered identically — no second summary of them — under one line saying an unattended run of this skill classified them and what it will do next: every thread answered, then a worktree on `headRefName`, one commit per ordinal, one push to that same branch, and one further comment when it finishes. Where no row was a fix it says that instead.

**This is the run's first write to the pull request**, and an unattended run posts this one and Step 7's and never a third. **A skip gets no comment of its own**: it stays skipped, its reason and evidence travel in this table, and it is answered in its own thread by the section below.
...
- **A thread holding one waits for Step 7**, so its reply can carry the commit that answered it — or say what stopped it. Nothing else is deferred.
...
| **skip** | **skip**, its reason spelled as Step 2 spells it, then that reason's evidence |
...
**A row whose entry carries no `threadId` is not replied to.** A review body and an issue comment have no reply primitive — there is nowhere to put one, and a fresh comment at the foot of the pull request is another comment rather than an answer. Their reasons travel in the table as they always have, and Step 7 names which they were.

### Then, the fix rows

- **None** ⇒ there is no code to write, and the table is the answer. The threads above are answered, no worktree is provisioned and nothing is pushed — an empty fix set has no commit to make, and a worktree provisioned for it is a checkout set up to write nothing in. Go to Step 7.
...
## Step 4 — a worktree on the pull request's own branch

**Nothing here creates a branch.** `headRefName` from Step 1's read is the branch the worktree checks out and the only branch this run will ever push to. **Every stop below reports git's message verbatim, leaves the worktree wherever it got to, and goes to Step 7** — which answers the threads it deferred and says what stopped it. Step 3 told those reviewers a fix was coming; a run that walks away from that in silence is one only its own channel heard about.
...
3. The checkout must sit at `origin/<headRefName>` — **checked, not assumed**. Where a stale local branch left it behind, `git -C <worktree> merge --ff-only origin/<headRefName>` catches it up; a refusal means the local branch diverged, so no push from it could be a fast-forward.
   ...
   Then `git -C <worktree> rev-parse HEAD` and `git -C <worktree> rev-parse origin/<headRefName>`, and **stop unless the two shas are equal**, reporting both and how they differ. The merge alone does not settle it: to a local branch *ahead* of the remote it says `Already up to date` and exits 0, and those unpushed commits — someone's work in progress, which the first bullet of step 2 attaches whatever state it is in — would be captured as the `base` below and then ride out on Step 6's push, landing on the pull request unreviewed and absent from Step 7's ledger.
4. **That HEAD sha is this run's `base`**, captured now, before anything is written to the branch. Step 5 reviews `base..HEAD` and Step 6 counts against it; the pull request's own base branch in its place would put the human's entire pull request under review, flooding findings about code this run did not write.
...
**Two profile keys, both read from `docs/agents/worktree.md`**: Setup command and Full-suite command. Under `gated`, one the file lacks is asked for **here**, under that profile's own ask-then-persist rule — ask once, write the answer in, never ask again — and here rather than earlier because a run stopping at Step 3 with no fix row needs neither, and a question hoisted above that stop spends the repository's one question on nothing. Under `unattended` nothing is asked and **nothing is written**: a profile lacking either key stops the run here like any other stop at this step, because neither has an honest default and the alternative is pushing work onto someone else's branch that nothing set up and nothing ran. This skill adds no key of its own, no second profile and no argument that changes what it does.

## Step 5 — the fixes, written here

The work is yours, in the worktree Step 4 attached: **no agent is dispatched and no workflow is run**. The table is the brief, and it is one you already wrote.

- **Implement the table's fix rows** — one commit per ordinal, in ordinal order, each against the clause its **Why** cell states and the bodies of the comments it covers, read whole. A row's clause is what a gated run's human approved, so it is implemented rather than reworded.
- **Use `/mattpocock-skills:tdd` where possible**, at the seams the comments themselves name.
- **Run typechecking regularly, single test files regularly, and the profile's Full-suite command once at the end**, from the worktree.
- **Once done, use `/mattpocock-skills:code-review`** over `base..HEAD`, the range Step 4 captured. **One pass is the bound**: its findings are applied once, in this run's last commits, and the review is not run again over them. A loop here has no gate in front of Step 6's push the way `/dev-loop`'s has in front of its own, and what it would be iterating on is somebody else's branch.
- **Commit to the branch the worktree already has** — never a new one, never an amend to a commit the branch already held, and never `--no-verify`.

Each message is a conventional-commit message — `<type>(<scope>): #<n> - <what changes>`, its type and scope taken from what that fix actually is — and every one of them is unique: two commits reading alike is how one is read as the other's work. `#<n>` is the pull request; GitHub numbers pull requests and issues in one sequence.

**Every anchor the table carries is a pre-run position.** GitHub numbered those lines before any of this ran, and the first commit's edit moves the second's — so each site is found by the content its comment quotes, the code and the symbols it names surviving an edit above them. Where `line` is null the anchor is `originalLine`, stale as the table marked it.

**Stopping short is a stop, not a smaller push.** A fix that cannot be made, a finding that cannot be applied, or a suite the work leaves red ends the writing here: report what stopped it, keep every commit already made in the worktree, and go to Step 7 with nothing pushed. `/dev-loop` pushes an ended sub-lane because the branch is its own; this one is a human's pull request, and a half-applied fix landing on it is precisely the state change on someone else's artifact this skill refuses.

## Step 6 — the push

**Step 5 decides this step before anything else is read**: a run that stopped short there pushes nothing and goes straight to Step 7. The clean path:

1. `git -C <worktree> rev-list --count <the sha from Step 4>..<headRefName>`. **Ask git, never the commits you believe you made** — the count is what settles the push.
2. **Zero** ⇒ nothing landed on the branch. Push nothing and go to Step 7, saying Step 5 reported no stop and made no commit.
...
The push is a fast-forward by construction: Step 4 started the worktree at `origin/<headRefName>` and stopped where it could not. **A rejection therefore means the branch moved while the fix was being written** — someone else pushed to the pull request. Report git's message verbatim, keep the worktree, and go to Step 7 saying nothing was pushed. Never retry harder, and never reach for a flag that would make it land anyway.

## Step 7 — the threads answered, and the conclusion commented back
...
**1. Ask git what reached the branch.** `git -C <worktree> log --oneline <the sha from Step 4>..<headRefName>`, read in the worktree, after the push and before Step 8 removes it — as Step 6 asks it for the count. What you believe you committed is a claim and the branch is the fact. A run with no worktree has nothing to ask.

**2. Answer the threads Step 3 deferred.** Every review thread holding a fix row, per Step 3's reply table, on every path that reaches this step. A pushed commit's reply carries its short sha and subject from that log; a path that pushed nothing says what stopped it in one clause instead, and never cites a sha the remote does not hold.

**3. Post one comment: the run's conclusion** — the ledger where it finished, the explanation where it ended. **It posts on every path that got past Step 3, in both modes** — under `gated` that means the gate was answered — **plus an unattended Step 1 ending**. Under `unattended` it is a second comment beside Step 3's table and never an edit to it; under `gated` it is the run's only one. A run stopped by Step 1's three refusals wrote nothing anywhere, is not this pull request's business, and still writes nothing here.

What it says — the commit list from step 1 above, and the rest from what this run did:
...
| every reply it left | one line per reply this run made, Step 3's and this step's alike — the row's `#` and the reply's url — then the rows that had no thread to reply in, named as carried by the table alone. A reply that failed is named here too, with what `gh` said |
| the table | Step 2's table as it stands, every row and every expansion, rendered identically. This copy is the one that outlives the run — a gated run's was a session's scrollback |
| the review | what the one `/mattpocock-skills:code-review` pass found: every finding applied, and every one deliberately not with the reason. A pass that found nothing says so |
| suite | the Full-suite command's result — `passed`, `failed` with its failing identifiers, or `not run` with why. A suite that did not run never reads as green |
| what stopped it | on a run that stopped before the push — the step, and its message verbatim |

`<planned>` counts the ordinals the table showed, and every planned commit the log does not hold is named as not made: a commit nobody wrote is the one thing a claimed list cannot show.

**No acceptance-criteria section**: there was no issue and so no spec axis, and a section rendering nothing claims something was judged.

**A run that stopped fills what it has.** It names the step that stopped it and carries that step's message verbatim — git's refusal to attach, the two shas that differed — and renders the table. **A section with nothing to put in it is left out**, for the reason the acceptance-criteria one is. A **Step 1** ending leaves out most of them and the table with them: there was nothing to classify, so it is the step, the read's message verbatim or the fact that the pull request held no unresolved comments, and nothing dressed up as more.

Either way it ends with what nothing else records, this session being the last thing that knows it: **the worktree, by path** — kept per Step 8, holding whatever was written before the run stopped, or plainly that none was attached where Step 4 stopped before it had one.
...
## Step 8 — the worktree, removed last
...
| Step 5 stopped short | untouched | **kept** |
...
**The branch is left alone either way, local ref and remote.** The remote one is what the pull request *is*; the local one may have been the human's before Step 4 attached to it.

**⟨notify⟩ Run conclusion.** Under `unattended`, the closing message goes after the disposal above — the run's last act, its token and reason from the Run mode section's table, paired with Step 1's `start`. Every path reaching this step sends it here, a run that stopped short at Step 5 and a rejected push included; a run that stopped at Step 1, 3 or 4 already sent its own there.

## Hard rules

- **One push; one comment under `gated` or two under `unattended`; one reply in each review thread the table covers; plus an unattended run's messages — nothing else leaves this session.** The push goes to the branch the pull request already has, and before Step 3 an unattended run writes only where it stops: Step 1's ending.
...
- **The fix work happens in your own context.** Nothing here dispatches an agent, and nothing here runs a workflow. The review is one `/mattpocock-skills:code-review` pass and its findings are applied once — a second pass would be a review loop with no human gate between it and a push onto somebody else's branch.
...
- **Worktree removal never passes --force.** Step 8's refusal on a dirty worktree IS the guard. `/dev-loop` and `/dev-loop-cleanup` state this same guardrail, deliberately — every skill that removes a worktree carries its own copy, because none of the three loads the others.
