...
**Every write ends with this line, and nothing visible after it:**

```markdown
<!-- replied from /pr-comments -->
```

The marker is invisible in GitHub's renderer and is how a later run recognises what this one already answered — the read below excludes every comment carrying it, against the matching pattern `read-comments.mjs` holds, so an edit to either form lands in both files or in neither. The one comment answering the rows with no thread writes a longer form of the same marker, named there.
...
- **`unattended` — post the table where the question would have stood.** Nobody is going to correct a row, so the table as classified is the one that counts. It goes on the pull request per **How this run writes**, rendered identically and never re-summarised, under one line saying an unattended run of this skill classified these comments and what it will do next. This is the run's first write; the only other comment such a run can post is the one answering the rows with no thread, below.
...
**Every review thread the table covers gets exactly one reply**, fix and skip alike, under both modes. A thread is one conversation, and two replies to it are two answers to one question; where the table covers several of its comments, the one reply names each and gives each its own line. **Reply in extremely concise, to the point, and using simple terms for the message.** A reply carries the row's Action — a skip's evidence with it, a disagreement's reason in plain words, its full reasoning staying beneath the table — plus, for a fix, the short sha and subject of the commit that made it, or in one clause what stopped it.

**When a thread is answered is decided by the latest thing its rows wait on.** A thread holding no fix row is answered **here**, the moment the table is the one that counts: those rows were settled at the gate, and a reply held to the end never arrives on a path that ends before it. A thread holding one waits for the push, so its reply can carry that commit. Nothing else is deferred.

**The rows whose entries carry no `threadId` share one comment on the pull request**, posted only where the table holds such a row, when the deferred replies go out. A review body and an issue comment have no reply primitive, so that comment gives each such row its own line in the reply shape above, and its marker names their ids — `<!-- replied from /pr-comments: <id> <id> -->` — so a later run's read excludes them as answered. Every other write takes the bare form.

**No fix row at all** ⇒ there is no code to write and the table is the answer: the threads above are answered, no worktree is provisioned, nothing is pushed, and the run goes straight to **The deferred writes, then the report**.

## The worktree, on the pull request's own branch

**Nothing here creates a branch of its own.** `headRefName` is the branch the worktree checks out and the only branch this run will ever push to. **Every stop below reports git's message verbatim, leaves the worktree wherever it got to, and goes to the deferred writes** — which answer the threads that were waiting and say what stopped it.
...
2. **Nothing** ⇒ nothing landed on the branch: push nothing, and say so in the report.
...
## The deferred writes, then the report

Three things, in this order — git before the replies, so a reply can name its commit; the report last, so it can say what landed.
...
2. **Answer what was deferred**, per the reply shape above: each fix thread, and the one comment for the rows with no thread, where the table holds any. A pushed commit's reply carries its short sha and subject from that log; a path that pushed nothing says what stopped it instead, and never cites a sha the remote does not hold.
3. **Report in this session, and post nothing.** Every comment was answered where it was raised, and a summary at the foot of the pull request would be a second answer to each. The report carries what reached the branch (that log, line for line, marked **not pushed** with why where nothing was), the suite's result, what the review pass changed and what it declined, and — where a run stopped early — the stage that stopped it and its message verbatim. It ends by naming the worktree by path wherever one was kept — `/dev-loop-cleanup` is its reaper — this session being the last thing that knows where the work is.
...
- **One push; one reply in each review thread the table covers; one comment for the rows with no thread, only where the table holds any, plus the table itself under `unattended` — nothing else leaves this session.** No summary, no conclusion. The push goes to the branch the pull request already has, and before the gate an unattended run writes only where it stops.
