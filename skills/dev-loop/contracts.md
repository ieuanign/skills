...
**Reporting replaces it, and it is the host's**, because the host has a shell and the phase scripts do not. Once the scripts have returned, the host reads the per-agent transcripts — the same instrument the baseline was measured with, and the only one carrying usage at all, the journal recording each agent's return value and no usage — and appends **one line per lane** to a single cost log in the scratch directory, keyed by issue number so the file answers what a given pull request cost. Not the issue thread, where cost would crowd a comment specified to be an extremely concise summary plus open questions.

**Every lane gets a line**, including one that ended and one that threw: improvement data collected only from the lanes that went well hides exactly the lanes worth reading. A lane nothing was measured for says so in words — a total of zero would read as free, and unmeasured is not free. Each line carries the total **and the per-stage split**, because the split is the whole value the baseline produced: a bare total says a lane was expensive, and the split says which dial to turn. The metric, the baseline figure the line prints a percentage against, and the line's format are `cost-log.mjs`'s; when the host runs it is SKILL.md's; and which runs write one at all is **Lane conclusion**'s branch below, not this section's.
