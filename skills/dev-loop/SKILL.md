...
4. Worktree preconditions:
   ...
   - **Neither remedy appends a line `.gitignore` already carries** — check the file first and skip the append if the exact line is present, reporting nothing. Appending is idempotent in effect and not on disk, so this is what keeps a check that misfires for some reason nobody has thought of yet from growing the file by one line per run.
...
3. Worktrees, per contracts.md's **worktree invariant** — removed when, and only when, the work reached the remote AND no human is expected to resume in it. Removal is `git worktree remove <WORKTREES>/<slug>` and is never `--force`, and it runs **only after that push succeeded**: after removal the remote branch is the only copy, so a push that failed or never ran keeps its worktree. A refusal means work was left behind — report `git -C <wt> status --porcelain` verbatim and keep that worktree; that refusal IS the dirty-work guard, so never argue with it. NEVER target MAIN: before any removal, confirm the path is NOT the first entry of `git worktree list`. The local branch and the plan file stay (`/dev-loop cleanup` reaps those once the PR merges). Which worktrees go:
   ...
   **Name what the removal destroys, then remove.** The refusal above is the guard for tracked modifications and untracked non-ignored files; it does not cover **ignored** files, which `git worktree remove` deletes without a word — so a lane's gitignored working material goes with its worktree, including anything the plan itself asked for. That destruction is correct and stays (contracts.md's invariant says why); what changes is that it is announced. Before removing, read the plan's **File touchpoints** — you already did at Gate 1, under both modes — and report every one that `git -C <wt> check-ignore -q <path>` calls ignored and that exists in the worktree, as paths going with the removal. Then remove. Nothing is copied out and nothing is kept: a path that must outlive its lane has to be committed, which stops it being ignored and makes this report empty again. Report nothing when the plan named no such path, which is the ordinary case — this is a targeted list and never `--ignored=matching`, whose every line would be the `node_modules` and copied-in config that provisioning put there on purpose.
