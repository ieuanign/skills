...
2. **The pipeline requires the Workflow tool.** Every stage is dispatched through it, so a session without it in your toolset → refuse the run, whatever the run mode, before a single agent is dispatched and before this Act asks the user anything else. Tell them the setting, `"enableWorkflows": true` in the per-machine settings file (`~/.claude/settings.json`), and that a **restart is required**. Asked once, then never again on this machine, with the settings file itself as the persistence:
   ...
   **It also requires the roster's preloaded skills.** Two of the agents you dispatch preload a skill of the declared dependency through their `skills:` frontmatter — `code-writer` takes `mattpocock-skills:tdd`, `debugger` takes `mattpocock-skills:diagnosing-bugs`. An entry that does not resolve is dropped SILENTLY: that agent launches without the method it was written around and returns less, which is indistinguishable from clean code. It resolves for whoever has the dependency installed and enabled, which is why it never breaks for the maintainer, and dies for the consumer whose install lacks it, has disabled it, or carries a version that renamed the skill. Look for each entry among your **own available skills**: only this session knows what actually resolved for the agents it is about to dispatch, where a manifest, a path or a subprocess knows only what is on disk. This is the introspection step 1 already performs for agent types, and it **asks nothing under either mode** — an entry you cannot find is a prerequisite this run cannot supply for itself, so under `gated` the run stops here naming the agent and the entry, and under `unattended` those same names go to the **⟨notify⟩ Intake refusal** below.

   **Name the remedy beside them**, and read it off the entry itself rather than off a package name or an install command, which are this plugin's identity and go stale the moment it is renamed or vendored. The entry's namespace prefix is the dependency it came from, and whether any sibling of that prefix resolved says which of the two remedies applies: where you can see **nothing** of that namespace, the dependency is not installed or is disabled — say to install the plugin that ships this skill, whose declared dependency is installed with it, or to enable that namespace where it is already there, and that a restart is required. Where you can see **other skills** of that namespace, it is installed and enabled and the entry is stale against the version it carries — nothing for the consumer to install, so say that instead, and that it is a defect to report against this plugin. A refusal naming only what broke withholds the one thing the reader cannot work out for themselves, and the two failures look identical from where they are standing.
