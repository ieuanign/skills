...
Mode W's per-sub-lane result carries a `terminal` of `{pr: 'ready'|'draft'|'none', push, reasons}` — contracts.md's terminal-state table, already applied to that sub-lane's own inputs. Carry it to Gate 2 unchanged: it is what step 2 below opens, and `reasons` is what a draft PR body explains itself with. Mode A needs no equivalent, because it implements the gated half only and there a human decides.
