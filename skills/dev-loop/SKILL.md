...
| **Repository profiles** (ask-then-persist, below) | every key in **Repo profile** below — branch template in `docs/agents/dev-loop.md`, suite command and **fix cycles** in `docs/agents/worktree.md` |
...
3. Compute the Derived facts and read both repo profiles (first run in a repo: ask-then-persist the branch template into `docs/agents/dev-loop.md` under `gated`; under `unattended` it takes its default and neither file is written).
...
- **Worktree removal never passes --force.** Step 3's refusal on a dirty worktree IS the guard. `/dev-loop-cleanup` and `/pr-comments` state this same guardrail, deliberately — every skill that removes a worktree carries its own copy, because none of the three loads the others.
- NEVER remove, force-modify, or `rm -rf` the main worktree (first entry of `git worktree list`). Worktree removal applies only to worktrees under `<WORKTREES>`, and only via `git worktree remove`.
