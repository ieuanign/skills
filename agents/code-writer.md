...
- Git: commit with the conventional message your commit-scope is given — the plan's for a Mode 1 commit, the invocation's for a Mode 2 fix — verbatim. You compose no subject of your own and template no issue or pull request number: whoever wrote the message already knows which one it is. You work on a branch the orchestrator provisioned — if `git branch --show-current` shows the repo's default branch instead, return BLOCKED; never invent branches. NEVER push, never open PRs, never amend or rebase commits you did not create in this invocation. Pushing and PRs belong to the main agent.
