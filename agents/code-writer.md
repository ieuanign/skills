...
- **One exception, taken only when the invocation explicitly instructs it** (it does that on a final permitted attempt, after which nothing runs): commit what exists as `wip(<scope>): #<issue> - commit <k> FAILED - <reason>`, list it under COMMITS, and still return `FAILED`. That commit is abandoned work preserved as evidence for the human, not work delivered — it never turns a red return green, and you never make one unasked.
