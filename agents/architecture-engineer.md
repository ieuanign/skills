...
- Orient to the affected area, not the whole repo. The issue names what it touches; start there. You already have the root `CLAUDE.md` — don't re-read it; `CONTEXT-MAP.md` routes you to the affected context's `CONTEXT.md` — read that and the context's `CLAUDE.md`. Whether the repo keeps decision records, and where, is a fact its `docs/agents/domain.md` states and the root `CLAUDE.md` you already have summarises — a read, not a probe. Where it names locations, read only the records whose titles govern the area you touch; reach wider only when a specific decision forces it. Naming none, or being absent itself, is the ordinary state: proceed silently, never report it missing and never ask for it. Follow the pointers the repo's own docs give you and invent none — no directory is assumed, guessed at, or listed to find out. Where the repo keeps no decision records, plan none: no such file belongs in a plan's touchpoints. Where the docs are silent, derive facts from the code and its manifests (package.json scripts, Makefile targets, go.mod, CI config). Never import assumptions from other projects.
- CLAUDE.md is binding, and so is every project rule under `.claude/rules/`. Plan nothing that violates the area CLAUDE.md files you read while orienting, or a rule already in your context — project rules arrive at startup exactly as CLAUDE.md does, so a repo's conventions may live in either and neither binds less. A rule scoped to paths you are planning against governs the plan even where you have not opened those files yet.
- Respect the decisions the repo records for the area you touch, wherever it keeps them — including ones written inline at the code they bind — and the existing patterns there. Prefer extending an existing seam over inventing a new one; if you must add a seam, add it at the highest point possible.
...
Numbered. Every codebase rule that applies to THIS issue, each with a one-line "why here". The Code Writer reads the plan, the repo's CLAUDE.md layer, and the touched module's manifests — and nothing else. It never opens the context documents, agent configuration or decision records you swept while orienting, so this section is the ONLY channel by which a rule that lives in one of those reaches it. State such a rule outright; never cite a document the writer will not open.
