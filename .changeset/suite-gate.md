...
Every ending the gate can produce is `UNRESOLVED`, never `HALT`: it runs only after the plan's commits exist and a review approved them, and the terminal categories turn on whether reviewable code exists. Halting there would discard a fully implemented, fully reviewed sub-lane and lose the human the PR that carries the failure to them.
