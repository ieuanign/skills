...
**Reporting replaces it**, and it belongs to the host, which has a shell. `cost-log.mjs` ships with the skill, reads its request on standard input like `notify.sh` does, and after the phase scripts return writes `.scratch/<project>/cost/<issue>.md` — one file per lane, in the same directory the run's plans live in, already gitignored in every repository this runs against. Deliberately not the issue: the per-lane comment is specified as an extremely concise summary plus open questions, and a cost table would bury it. There is deliberately no way to pass the target in: the skill's configuration rule refuses a per-run override of cost behaviour, and a target that can be passed is one.
